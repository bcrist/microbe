...
fn make(step: *Build.Step, progress: *std.Progress.Node) !void {
    ...
    if (raw_boot2.len == 0) {
        std.log.err(".boot2_src section is empty; did you forget to export a boot2 function?", .{});
        return error.InvalidBoot2Section;
    } else if (raw_boot2.len > 252) {
        if (raw_boot2.len > 4000) {
            std.log.err("Expected .boot2_src section to be <= 252 bytes; found > 4000", .{});
        } else {
            std.log.err("Expected .boot2_src section to be <= 252 bytes; found {}", .{ raw_boot2.len });
        }
        return error.InvalidBoot2Section;
    }
    ...
    try writer.writeAll(
        ...
        \\pub export const _boot2: [256]u8 linksection(".boot2") = .{
        \\
    );
